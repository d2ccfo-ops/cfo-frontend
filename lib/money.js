...
// PAISE, not rupees. Every money field the API sends is a paise bigint
// serialised as a string — `netRevenueMinor`, `amountPaise`, `expected` — so
// the conversion belongs here rather than at forty call sites, where one
// forgotten `/100` renders ₹4,890 as ₹4,89,000 and looks entirely plausible.
//
// Takes a string or a number: a paise figure past 2^53 would lose precision as
// a JS number, and the strings are exactly why the API sends them that way.
export function formatInrMinor(paise) {
  if (paise == null || paise === "") return "—";
  const n = typeof paise === "bigint" ? Number(paise) : Number(paise);
  if (Number.isNaN(n)) return "—";
  return formatInrShort(n / 100);
}
